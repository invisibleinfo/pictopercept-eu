...
We measure what Dutch citizens picture when they think about the EU, using PictoPercept: respondents see two images side by side, answer one fixed question under time pressure, and select one. Repeated across systematically varied images, the choices reveal which attributes (frame x valence) drive this selection. That distribution can then be compared against the EU's own social media output.

**Design: `frame` (6 levels) × `valence` (2 levels) = 12 cells.**

| Frame | Positive | Negative |
|---|---|---|
| **institutional** | EU flag, blue with the circle of yellow stars; map of Europe with member states in blue and internal borders fading out; semi-circular parliament hemicycle, microphones, interpreter booths, full seats; euro coin; Brussels EU quarter with modern glass buildings, a long row of flags, wide open squares; a summit handshake at a podium; an official signing ceremony | Bureaucracy / redtape / piles of documents; Empty hemicycle, rows of vacant seats; a long anonymous corridor of identical closed doors; stacks of binders and paperwork; a glass facade seen from outside behind security fencing; a single citizen dwarfed by monumental architecture; a weathered flag with a faded or missing star; a map where the Netherlands is a small sliver among larger blocs; a revolving door; a lectern facing an empty room |
...
## Task 2: put the images into Qualtrics as trials
